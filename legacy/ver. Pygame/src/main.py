...
class MineArea():
    ...
    def change_layout(self, width=9, height=9, mine_num=10):
        ...
        self.tot_display_width = self.grid_width * (self.cell_side_length + self.cell_gap) - self.cell_gap + self.border * 2
        ...
        self.display_pad_width = (self.cell_side_length + self.cell_gap) * self.grid_width - self.cell_gap
        ...
        self.mine_sweep_width = (self.cell_side_length + self.cell_gap) * self.grid_width - self.cell_gap
        self.mine_sweep_height = (self.cell_side_length + self.cell_gap) * self.grid_height - self.cell_gap
        ...
        # tot area
        self.tot_display_height = self.grid_height * (self.cell_side_length + self.cell_gap) - self.cell_gap + self.border * 2 + self.display_pad_height + self.pad_sweep_gap_height
    ...
    def generate_mines(self,click_x,click_y) -> None:
        """
        生成地雷分布（在第一次点击后调用）
        
        :param click_x: 点击X坐标（行索引）
        :param click_y: 点击Y坐标（列索引）
        """
        # 防止溢出，雷区总雷数不能超过总格子数减去9
        max_mines = self.grid_width * self.grid_height - 9
        if self.mine_num > max_mines:
            print(f"错误：雷数超过最大允许值 {max_mines}，已阻止开始游戏")
            return
        
        # 确保 点击处 3x3范围内 无雷
        safe_zone = [(click_x + dx, click_y + dy) 
                    for dx in (-1, 0, 1) 
                    for dy in (-1, 0, 1)
                    if 0 <= click_x + dx < self.grid_height
                    and 0 <= click_y + dy < self.grid_width]
        ...
        # 从候选列表中随机选择雷的位置
        mine_positions = random.sample(candidates, self.mine_num)
        for x, y in mine_positions:
            self.mine_board[x][y] = '*'
        
        self._calculate_nums()  # 计算每个格子周围的地雷数量

    def _calculate_nums(self) -> None:
        """计算每个格子周围的地雷数量"""
            ...
            for y in range(self.grid_width):
                ...
                for dx in (-1, 0, 1):
                    for dy in (-1, 0, 1):
                        nx, ny = x + dx, y + dy
                        # 如果越界，则跳过
                        if nx < 0 or nx >= self.grid_height or ny < 0 or ny >= self.grid_width:
                            continue
                        # 如果该位置是雷，则计数加1
                        if self.mine_board[nx][ny] == '*':
                            cnt += 1
                # 如果周围没有雷，则设置为空格，否则设置为雷的数量
                self.mine_board[x][y] = str(cnt) if cnt > 0 else ' '
        ...
        for i in range(self.grid_height):
            for j in range(self.grid_width):
                ...
                if self.mine_board[i][j] == '*': # 真实层为雷
                    ...
                    image = self.images_hidden_number[self.mine_board[i][j]]
                    self.surface.blit(image,self.mine_cell_rect)
    ...
    def restart(self):
        self.change(self.grid_width,self.grid_height,self.mine_num)
    ...
    def _run_basic_rules(self):
        ...
        for i in range(self.grid_height):
            for j in range(self.grid_width):
                ...
                cnt_signed = 0
                # 3*3区域内扫描
                for x in range(max(0, i - 1), min(self.grid_height, i + 2)):
                    for y in range(max(0, j - 1), min(self.grid_width, j + 2)):
        ...
        for i in range(self.grid_height):
            for j in range(self.grid_width):
                ...
                for dx, dy in directions:
                    ni, nj = i+dx, j+dy
                    # 如果不在范围内，跳过
                    if not (0 <= ni < self.grid_height and 0 <= nj < self.grid_width):
                        continue
    ...
    def _process_semi_auto(self, x, y):
        ...
        # 统计标记数量
        sign_cnt = 0
        for i in range(max(0, x - 1), min(self.grid_height, x + 2)):
            for j in range(max(0, y - 1), min(self.grid_width, y + 2)):
    ...
    def clear_blank(self,x,y):
        visited_area = [[0 for _ in range(self.grid_width)] for _ in range(self.grid_height)]
        def clear(i,j):
            ...
            clear(min(i + 1, self.grid_height - 1), j)
            clear(min(i + 1, self.grid_height - 1), min(j + 1, self.grid_width - 1))
            clear(min(i + 1, self.grid_height - 1), max(j - 1, 0))
            clear(max(i - 1, 0),j)
            clear(max(i - 1, 0), min(j + 1, self.grid_width - 1))
            clear(max(i - 1, 0), max(j - 1, 0))
            clear(i, min(j + 1, self.grid_width - 1))
            clear(i, max(j - 1, 0))
...
if __name__  == '__main__':
    ...
    # 自定义模式下调节宽、高、雷数按钮
    button_width = CustomButton('宽',screen,init_num=mine_area.grid_width,left=Config.SIZE["SCREEN_WIDTH"] // 2 - 96 - 22 - 3 - 96)
    button_height = CustomButton('高',screen,init_num=mine_area.grid_height,left=Config.SIZE["SCREEN_WIDTH"] // 2 - 96)
